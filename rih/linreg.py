...
def linreg_score(gdf_cbsa_bg: pd.DataFrame, year: int, group_lh_together: bool) -> Dict:
    ...
    mape_score_log = 1 - mean_absolute_percentage_error(
        y_true=y_test,
        y_pred=np.exp(model_log.predict(X_test)),
        sample_weight=w_test
    )
    ...
    return {
        ...
        "full": {
            "coefficients": {col: coefficient for col, coefficient in zip(X_train.columns, model.coef_.tolist())},
            "intercept": float(model.intercept_),
            "r2_score": float(score),
            "mape_score": float(mape_score),
        },
        "one": {
            "coefficients": {col: coefficient for col, coefficient in zip(X_train_1.columns, model_1.coef_.tolist())},
            "intercept": float(model_1.intercept_),
            "r2_score": float(score_1),
            "mape_score": float(mape_score_1),
        },
        "full_log": {
            "coefficients": {col: coefficient for col, coefficient in zip(X_train.columns, model_log.coef_.tolist())},
            "intercept": float(model_log.intercept_),
            "r2_score": float(score_log),
            "mape_score": float(mape_score_log),
        },
        "one_log": {
            "coefficients": {col: coefficient for col, coefficient in zip(X_train_1.columns, model_log_1.coef_.tolist())},
